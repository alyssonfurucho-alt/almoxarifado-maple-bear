  ...
  async function confirmar() {
    ...
    for (const item of lista) {
      try {
        ...
        // PASSO 1: produto não existe pelo EAN → cria na tabela produtos
        if (!produtoId && item.cean) {
          const { data: novoProd, error: errProd } = await supabase
            .from('produtos')
            .insert({ codigo_barras: item.cean, nome: item.nome, ativo: true })
            .select()
            .single()
        ...
        // PASSO 2: item de estoque já existe → atualiza quantidade e custo
        if (item.existente) {
          const novaQtd = (item.existente.quantidade || 0) + item.qtd
          const { error: errUpd } = await supabase
            .from('itens')
            .update({
              quantidade: novaQtd,
              custo_unitario: item.custo,
              ...(produtoId && !item.existente.produto_id ? { produto_id: produtoId } : {}),
            })
        ...
        // PASSO 3: item de estoque não existe → cria novo
        } else {
          const { data: novoItem, error: errIns } = await supabase
            .from('itens')
            .insert({
          ...
          itensLog.push({ id: novoItem.id, nome: item.nome, qtd: item.qtd, tipo: 'cadastro' })
          cadastrados++
        }
      } catch (e) {
        console.error('Erro ao importar item:', item.nome, e)
        erros++
      }
    }
