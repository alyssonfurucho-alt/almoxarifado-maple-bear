...
export default function ImportarNFe() {
  ...
  const [linhas, setLinhas]               = useState([])      // itens da prévia
  ...
  async function lerXML(xmlStr) {
    try {
      ...
      // busca produtos e itens existentes no banco
      ...
      for (const det of dets) {
        ...
        if (produto) {
          itemEstoque = (itensDB||[]).find(i => i.produto_id === produto.id)
        }
        if (!itemEstoque && ean) {
          itemEstoque = (itensDB||[]).find(i => i.codigo_barras === ean)
        }
        if (!itemEstoque) {
          itemEstoque = (itensDB||[]).find(i =>
  ...
  async function confirmar() {
    ...
    for (const linha of lista) {
      try {
        ...
        if (linha.itemEstoque) {
          ...
          const { data: novoItem, error: errI } = await supabase
            .from('itens')
            .insert({
          ...
          log.push({ id: novoItem.id, nome: nomeItem, qtd: linha.qtd, tipo: 'cadastro' })
          qtdCadastrados++
        }
    ...
    setSaving(false)
    setResultado({ cadastrados: qtdCadastrados, atualizados: qtdAtualizados, erros: qtdErros })
    setEtapa('sucesso')
  }
  ...
  async function desfazerNota(nota) {
    ...
    try {
      ...
      for (const it of itensLog) {
        ...
        if (!item) continue
        await supabase.from('itens').update({ quantidade: Math.max(0, item.quantidade - it.qtd) }).eq('id', it.id)
