...
export default function Cobranca() {
  ...
  async function load() {
    ...
    const { data:devAvarias } = await supabase.from('devolucoes')
      .select('*, saidas(professor_nome_snapshot, turma_codigo_snapshot, professores(nome,registro), turmas(codigo,turno), itens(nome))')
  ...
  const TabelaCobranca = ({ rows, tipo }) => (
    <div className="card">
      <table>
        <thead><tr><th>Item</th><th>Qtd pendente</th><th>Professor(a)</th><th>Registro</th><th>Turma</th><th>Turno</th><th>Data saída</th><th>Prazo</th><th>{tipo==='venc'?'Atraso':'Restam'}</th><th>Valor</th></tr></thead>
        <tbody>
          {rows.map(s=>{
  ...
  return (
    <div>
      ...
      <div className="card">
        <table>
          <thead><tr><th>Item</th><th>Qtd avariada</th><th>Professor(a)</th><th>Registro</th><th>Turma</th><th>Turno</th><th>Data devolução</th><th>Descrição da avaria</th></tr></thead>
          <tbody>
            {avarias.map(d=>{
              const s = d.saidas
